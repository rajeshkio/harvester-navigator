...
// GetPodFromVolumeEnhanced works with any CSI driver, with fallback to Longhorn-specific method
func GetPodFromVolumeEnhanced(client *kubernetes.Clientset, volumeDetails *VolumeDetails) (string, error) {
	if !volumeDetails.IsLonghornCSI {
		return getPodFromPVC(client, volumeDetails.PVCName, volumeDetails.Namespace)
	}

	if volumeDetails.BackendDetails != nil {
		return GetPodFromVolume(volumeDetails.BackendDetails)
	}
